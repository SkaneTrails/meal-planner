...
- All recipes assigned to Hemmestorp with visibility="shared"
"""
...
HEMMESTORP_ID = "IXdzHJ91NZeutylohx1t"
...
def migrate_and_link(*, dry_run: bool = True):
    ...
    4. Set all to Hemmestorp + visibility='shared'
    """
    print("\n" + "=" * 60)
    ...
    for doc_id, _enhanced_data in analysis["mp_enhanced"]:
        ...
        original_data["household_id"] = HEMMESTORP_ID
    ...
    for doc_id, data in analysis["only_in_default"]:
        ...
        data["household_id"] = HEMMESTORP_ID
    ...
    for doc_id, data in analysis["mp_recipes"]:
        if data.get("household_id") == HEMMESTORP_ID:
            continue

        if dry_run:
            pass
        else:
            mp_db.collection("recipes").document(doc_id).update({"household_id": HEMMESTORP_ID, "visibility": "shared"})
        updated += 1
