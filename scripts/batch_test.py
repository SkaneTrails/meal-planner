...
from recipe_enhancer import get_recipe, enhance_recipe
...
def main():
    ...
    for i, (recipe_id, name) in enumerate(RECIPES, 1):
        ...
        if not original:
            print(f"  ERROR: Recipe not found")
        ...
        print(f"  Decimals (0.5): {'Yes' if result['has_decimals'] else 'No'} {'(BAD!)' if result['has_decimals'] else ''}")
