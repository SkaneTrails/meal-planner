...
async def list_notes(user: Annotated[AuthenticatedUser, Depends(require_auth)], recipe_id: str) -> list[RecipeNote]:
...
async def create_note(
    user: Annotated[AuthenticatedUser, Depends(require_auth)], recipe_id: str, body: RecipeNoteCreate
) -> RecipeNote:
    """Add a note to a recipe for the user's household."""
    household_id = require_household(user)
    return recipe_notes_storage.create_note(
        recipe_id=recipe_id, household_id=household_id, text=body.text, created_by=user.email
    )


@router.delete("/{recipe_id}/notes/{note_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_note(user: Annotated[AuthenticatedUser, Depends(require_auth)], recipe_id: str, note_id: str) -> None:
    """Delete a note by ID. Only household members can delete their household's notes."""
    household_id = require_household(user)
    if not recipe_notes_storage.delete_note(note_id, household_id, recipe_id):
