...
export default function RecipeDetailScreen() {
  ...
  // Fetch original version when toggle is on and recipe has enhanced_from
  const originalId = recipe?.enhanced_from;
  const { data: originalRecipe } = useRecipe(originalId ?? '');
  const displayRecipe = viewingOriginal && originalRecipe ? originalRecipe : recipe;
        ...
        <View style={{
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 12, alignItems: 'center' }}>
            ...
            {recipe?.enhanced_from && (
              ...
              >
                <Ionicons
                  name={!viewingOriginal ? 'sparkles' : 'document-text-outline'}
                  size={16}
                  color={!viewingOriginal ? '#7C3AED' : colors.text.inverse}
                />
                <Text style={{
