...
export default function GroceryScreen() {
    ...
    React.useCallback(() => {
      const loadData = async () => {
        try {
          ...
          console.log('[Grocery] Raw data from storage:', {
            customData,
            mealsData,
            servingsData,
          });
  ...
  useEffect(() => {
    ...
    selectedMealKeys.forEach((key) => {
      ...
      console.log(
        `[Grocery] Found recipe:`,
        recipe?.title,
        `with ${recipe?.ingredients.length} ingredients`,
      );
  ...
  useEffect(() => {
    if (!isLoading) {
      AsyncStorage.setItem('grocery_custom_items', JSON.stringify(customItems))
        .then(() =>
          console.log('[Grocery] Saved custom items:', customItems.length),
        )
        .catch((error) =>
  ...
  const checkedItemsToBuy = useMemo(() => {
    return groceryListWithChecked.items.filter(
      ...
      <View style={{ flex: 1 }}>
        ...
        {totalItems > 0 ? (
          ...
          >
            ...
            >
              Go to Weekly Menu and tap "Create List" to generate your grocery
              list from planned meals
            </Text>
          </View>
        )}
      </View>
