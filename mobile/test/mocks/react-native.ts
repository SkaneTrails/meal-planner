...
export const Animated = {
  ...
  spring: (value: any, config: any) => ({ start: (cb?: any) => { if (value?.setValue) value.setValue(config?.toValue ?? 0); cb?.({ finished: true }); }, stop: () => {} }),
  timing: (value: any, config: any) => ({ start: (cb?: any) => { if (value?.setValue) value.setValue(config?.toValue ?? 0); cb?.({ finished: true }); }, stop: () => {} }),
