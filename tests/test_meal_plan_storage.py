...
class TestMealPlanConversion:
    ...
    def test_meal_plan_to_firestore_empty(self) -> None:
        """Test converting empty meal plan to Firestore format."""
        result = _meal_plan_to_firestore({})
        assert result["meals"] == {}
        assert "updated_at" in result

    def test_meal_plan_to_firestore_with_meals(self) -> None:
        """Test converting meal plan with meals to Firestore format."""
        meal_plan = {("2025-01-15", "breakfast"): "recipe_123", ("2025-01-15", "lunch"): "custom:Sandwich"}
    ...
    def test_firestore_to_meal_plan_empty(self) -> None:
        """Test converting empty Firestore data to meal plan."""
        result = _firestore_to_meal_plan({})
        assert result == {}

    def test_firestore_to_meal_plan_with_meals(self) -> None:
        """Test converting Firestore data with meals to meal plan."""
        data = {"meals": {"2025-01-15_breakfast": "recipe_123", "2025-01-15_dinner": "recipe_456"}}
...
class TestDayNotes:
    ...
    def test_load_day_notes_empty_doc(self, mock_get_client: MagicMock) -> None:
    ...
    @patch("app.storage.meal_plan_storage.get_firestore_client")
    def test_load_day_notes_no_notes(self, mock_get_client: MagicMock) -> None:
    ...
    @patch("app.storage.meal_plan_storage.get_firestore_client")
    def test_load_day_notes_with_notes(self, mock_get_client: MagicMock) -> None:
        """Test loading notes when document has notes."""
        mock_db = MagicMock()
        mock_get_client.return_value = mock_db
        mock_doc = MagicMock()
        mock_doc.exists = True
        mock_doc.to_dict.return_value = {"meals": {}, "notes": {"2025-01-15": "office", "2025-01-16": "home"}}
        mock_db.collection.return_value.document.return_value.get.return_value = mock_doc
    ...
    @patch("app.storage.meal_plan_storage.get_firestore_client")
    def test_update_day_note_set(self, mock_get_client: MagicMock) -> None:
    ...
    @patch("app.storage.meal_plan_storage.get_firestore_client")
    def test_update_day_note_delete(self, mock_get_client: MagicMock) -> None:
